...
int B_PLUS_TREE_INTERNAL_PAGE_TYPE::InsertNodeAfter(const ValueType &old_value, const KeyType &new_key,
  ...
  array[old_idx + 1] = std::make_pair(new_key, new_value);
  IncreaseSize(1);
  return GetSize();
}
...
void B_PLUS_TREE_INTERNAL_PAGE_TYPE::MoveHalfTo(BPlusTreeInternalPage *recipient,
  ...
  recipient->CopyNFrom(array + min_size, size - min_size, buffer_pool_manager);
  IncreaseSize(min_size - size);
}
...
void B_PLUS_TREE_INTERNAL_PAGE_TYPE::CopyNFrom(MappingType *items, int size, BufferPoolManager *buffer_pool_manager) {
  ...
  for (int i = 0; i < size; i++) {
    ...
    auto page = reinterpret_cast<BPlusTreeInternalPage *>(buffer_pool_manager->FetchPage(page_id));
...
void B_PLUS_TREE_INTERNAL_PAGE_TYPE::MoveAllTo(BPlusTreeInternalPage *recipient, const KeyType &middle_key,
                                               BufferPoolManager *buffer_pool_manager) {
  int idx = recipient->GetSize();
  recipient->CopyNFrom(array, GetSize(), buffer_pool_manager);
  recipient->array[idx].first = middle_key;
  SetSize(0);
}
...
void B_PLUS_TREE_INTERNAL_PAGE_TYPE::MoveFirstToEndOf(BPlusTreeInternalPage *recipient, const KeyType &middle_key,
                                                      BufferPoolManager *buffer_pool_manager) {
  int idx = recipient->GetSize();
  recipient->CopyNFrom(array, 1, buffer_pool_manager);
  recipient->array[idx].first = middle_key;
...
void B_PLUS_TREE_INTERNAL_PAGE_TYPE::CopyFirstFrom(const MappingType &pair, BufferPoolManager *buffer_pool_manager) {
  ...
  array[0] = pair;
  auto page = reinterpret_cast<BPlusTreeInternalPage *>(buffer_pool_manager->FetchPage(pair.second));
  page->SetParentPageId(GetPageId());
